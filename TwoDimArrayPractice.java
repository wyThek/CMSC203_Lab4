...
public class TwoDimArrayPractice extends JFrame {
    ...
    public int findMinimum(int column) {
        ...
        int minimum = 999; // the minimum value is initiated to greater than 80 to avoid logical error
        // loop through rows
        for (int row = 0; row < intArray.length; row++) {
            // if the value is smaller than the minimum, replace it
    ...
    public int countFound(int value) {
        ...
        for (int row = 0; row < intArray.length; row++) {
            ...
            for (int column = 0; column < intArray[row].length; column++) {
                // if the input value is same as the value in the the array, increase num by 1
                if (intArray[row][column] == value) {
                    num++;
                    animate(row, column, num); // this is cool haha
                }
            }
        }
        return num; // return the counter
    }
